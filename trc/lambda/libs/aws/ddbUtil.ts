    ...
    ConditionExpression = ConditionKeys
      .map((key, index) => {
        ...
        if (key.startsWith('>=') || key.startsWith('<=') || key.startsWith('<>')) {
          sign = key.substring(0, 2);
          trimmedKey = key.slice(2);
        } else {
          sign = key[0];
          trimmedKey = key.slice(1);
        }

        // ExpressionAttributeNames[`#${trimmedKey}_condition`] = trimmedKey;
        ExpressionAttributeValues[`:${trimmedKey}_condition`] = ConditionValues[index];
        return `${trimmedKey} ${sign} :${trimmedKey}_condition`;
      })
      .join(', ');
  }
