...
interface ColumnDefinition {
  ...
  searchable: boolean;
  projection?: boolean;
  type?: string; // e.g. "String" | "Integer" | "Float" — non-string types are excluded from LIKE search
}
  ...
  private order(): string {
    ...
    this.request.order.forEach(({ column, dir }) => {
      ...
      if (requestColumn.orderable === "true") {
        orderBy.push(`${columnDef.db} ${dir.toUpperCase()} NULLS LAST`);
      }
    });
  ...
  private filter(): string {
    ...
    if (this.request.search && this.request.search.value.trim().length > 1) {
      ...
      this.columns.forEach((col) => {
        const isStringType = !col.type || col.type.toLowerCase() === "string";
        if (col.searchable && isStringType) {
