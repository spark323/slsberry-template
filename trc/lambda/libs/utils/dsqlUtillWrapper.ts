...
async function generateToken(credential?: AwsCredentialIdentityProvider) {
    ...
    let dsqlObject: any = {
        hostname: process.env.stage == "prod" ? process.env.dsql_endpoint! : process.env.dsql_endpoint_dev!,
...
async function getNewDBInstance(credential?: AwsCredentialIdentityProvider): Promise<Knex> {
    ...
    newDB = knex({
        client: 'pg',
        connection: {
            host: process.env.stage == "prod" ? process.env.dsql_endpoint! : process.env.dsql_endpoint_dev!,
            user: 'admin',
            password: token,
            port: 5432,
            database: 'postgres',
